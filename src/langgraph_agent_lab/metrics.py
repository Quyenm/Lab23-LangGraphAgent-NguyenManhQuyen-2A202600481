"""Metrics schema and computation helpers.

MetricsReport is the grading contract. All fields are required.
scenario_metrics must have >= 6 entries to pass validation.
"""
...
def metric_from_state(
    ...
    """Extract a ScenarioMetric from a completed run state."""
    events = state.get("events", []) or []
    errors = state.get("errors", []) or []
    actual_route = state.get("route")
    approval = state.get("approval")

    nodes = [e.get("node", "unknown") for e in events]
    retry_count = sum(1 for n in nodes if n == "retry")
    interrupt_count = sum(1 for n in nodes if n == "approval")

    # Success: correct route AND has a final answer/pending question
    has_output = bool(state.get("final_answer") or state.get("pending_question"))
    route_match = actual_route == expected_route
    approval_ok = (not approval_required) or (approval is not None and approval.get("approved"))

    success = route_match and has_output and approval_ok
...
def summarize_metrics(
    ...
    """Aggregate per-scenario metrics into a report."""
    if not items:
